...
**Result**: sapply() creates a vector (or a one-dimensional structure) that contains: - The mean of each numeric element from the list. - NA for elements that are not numeric.
...
nested_li <- list(my_list = my_list, log_vec = log_vec)
nested_li
```
...
**Question**\
Create another list, `additional_li`, containing: 1. A 3x3 matrix `matrix_sample` (values 1 to 9)\
...
```{r}
# Your code here:
matrix_sample <- matrix(1:9, 3, 3);
Name <- c("A" , "B", "C")
Age <- c(1, 2, 3)
df_additional <- data.frame(Name = Name, Age = Age)
additional_li <- list(matrix_sample, df_additional)
combined_li <- list(c(nested_li, additional_li))
combined_li
```
