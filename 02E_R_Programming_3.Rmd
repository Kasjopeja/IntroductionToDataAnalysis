...
While loops are fundamental control structures in R that allow you to execute a block of code repeatedly as long as a specified condition remains `TRUE`.
They are particularly useful for scenarios where the number of iterations is not known beforehand and depends on dynamic conditions during runtime.
...
**Explanation:** - The loop starts with `x` equal to 0.
...
**Explanation:** - Similar to the previous example, but an `if` statement checks if `x` has reached 10.
- When `x` equals 10, a message is printed.
However, the loop still relies on the `while` condition to terminate.
...
**Explanation:** - The loop behaves similarly to the previous example.
...
**Explanation:** - The loop iterates over each element in vector `x`.
...
Practice is essential to reinforce your understanding of `while` loops in R.
Complete the following exercises by filling in the R code where indicated.
...
**Question:** Write a `while` loop that continuously prompts the user to enter a number.
The loop should terminate when the user enters a negative number.
Keep track of the sum of all entered numbers (excluding the negative number).
...
**Question:** Write a `while` loop that generates random numbers between 0 and 1 using `runif(1)` and appends them to a vector.
The loop should stop when the cumulative sum of the vector exceeds 5.
...
**Question:** Write a `while` loop that generates random numbers between 0 and 1 using `runif(1)` and stops when a number greater than 0.9 is generated.
...
**Question:** Create a list of mixed data types (numeric, character, logical).
Use a `while` loop to iterate through the list and print only the numeric elements.

```{r}
l <- list(2, "one", FALSE, 3.5, "four", "B", 1)
i <- 1
...
**Question:** Write a `while` loop that accumulates even numbers starting from 2, adding 2 in each iteration, until the accumulated sum exceeds 100.
Print each even number as it's added.
...
**Question:** Write a `while` loop that allows a user to guess a secret number (e.g., 7).
The user has up to 5 attempts to guess correctly.
After each incorrect guess, inform the user and decrement the remaining attempts.
If the user guesses correctly, congratulate them and exit the loop.
If the user fails to guess within 5 attempts, reveal the secret number.
...
while (i < 5) {
  answer <- as.numeric(readline(prompt = "Give a number: "))
  if (secret == answer) {
...
**Question:** Write a `while` loop that generates the Fibonacci sequence until the next number in the sequence exceeds 1000.
Store the sequence in a vector and print it at the end.

```{r}
f <- c(1,1)
i <- 2

while (f[[length(f)]] < 1000) {
  f <- c(f, f[[i]] + f[[i-1]])
  i <-  i + 1
  print(f[[length(f)]])
}
...
In this session, we explored the fundamentals of `while` loops in R, including their syntax, usage, and best practices.
We examined practical examples demonstrating how to implement `while` loops for various tasks, integrated conditional statements within loops, and utilized loop control mechanisms like `break` and `next`.
Through the exercises, you have practiced creating and manipulating `while` loops to solve real-world problems, enhancing your programming proficiency in R.

**Next Steps:** - Continue practicing with `while` loops to build confidence.
